...
export const CourseReviews: React.FC<CourseReviewsProps> = ({
  ...
  // Lưu các hình ảnh đã được chọn
  const selectedImages = new Set();

  function getRandomImage() {
    const imageLength = imageRandom.filter((item) => !selectedImages.has(item))
    const randomIndex = Math.floor(Math.random() * imageLength.length);
    selectedImages.add(imageLength[randomIndex]);
    return imageLength[randomIndex];
  }
